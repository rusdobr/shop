...
function pricessCategoriesNew()
{
    ...
    $result = [];
    gatherRows($data, -1, $result);
    return $result;
}


function gatherRows($root, $level, array &$result)
{
    if (empty($root)) {
        return;
    }
    ...
    foreach ($root['categories'] as $category) {
        gatherRows($category, $level + 1, $result);
    }
}
...
if (isset($_GET["show_price"])) //show pricelist
{
    ...
    $pricelist_elements = pricessCategoriesNew();
